...
Maximum-likelihood (ML) learning in energy-based Markov random fields is hard because the gradient needs expectations under the model distribution, which typically requires long MCMC runs to equilibrium. Hinton’s contrastive divergence (CD) idea runs the Markov chain only a few steps starting from the data; it often works well empirically. This paper studies CD’s **fixed points** and **bias**: (i) CD is **biased in general** (its fixed points usually differ from ML’s), but (ii) in empirical tests on small Boltzmann machines where exact computations are possible, the bias is **typically very small**, and (iii) a practical strategy is **CD for most training + a short exact-ML phase** (“CD-ML”) to remove the residual bias at low extra cost. 
...
= -\left\langle \frac{\partial E}{\partial W}\right\rangle_{0}

* \left\langle \frac{\partial E}{\partial W}\right\rangle_{\infty},
  ]
...
ML minimizes $KL(p_0\|p_\infty)$. CD learning approximately follows the gradient of:
...
## 2) Models studied and the exact ML vs exact CD(_n) rules

They focus on **Boltzmann machines** with binary units (\in{0,1}), no biases (for simplicity). Two cases: fully visible BM (VBM) and restricted BM (RBM). 

### 2.1 Fully Visible Boltzmann Machine VBM((v))

* (v) visible units (x=(x_1,\dots,x_v)^T), no hidden units.
...
$$w_{ij}^{(\tau+1)}=w_{ij}^{(\tau)}+\eta\Big(\big\langle\langle y_i x_j\rangle_{p(y|x;W)}\big\rangle_{0}-\langle y_i x_j\rangle_\infty\Big).$$


**Exact CD(_n) update:**
[
w_{ij}^{(\tau+1)}=w_{ij}^{(\tau)}+\eta\Big(\big\langle\langle y_i x_j\rangle_{p(y|x;W)}\big\rangle_{0}-\langle y_i x_j\rangle_n\Big).
]
...
* and the **exact** distribution after (n) MCMC steps (p_n),
  at each learning iteration. ML means (n\to\infty); CD(_n) means finite (n) but computed exactly. 
...
* CD(*1)-fixed-point data distributions:
...
* (p_{11}=1/4),
* (3p_{01}+p_{11}=1),
  which indeed has measure zero in the simplex. So for almost every data distribution (p_0), CD(_1) is biased relative to ML. 
...
* define ML as exact (n\to\infty), CD(_n) as exact (n)-step chain,
...
* stop when gradient norm criterion ( |e| < 10^{-7}), with
  ...
  (e=\langle x_i x_j\rangle_0-\langle x_i x_j\rangle_1) for CD(_1). 
...
Histograms of $KL(p_0\|\|p_{ML})$ and $KL(p_0\|\|p_{CD})$ are very similar; CD is “very close to ML on average” (Figure 2). 
...
**Key takeaway from VBM experiments:** CD(_1) is biased in theory, but the **magnitude** is usually small; and ML iterations are much more expensive in practical MCMC settings, so CD’s speed/variance advantages can dominate. 
...
* Assume 1 ML iteration costs 20× a CD iteration (their “reasonable estimate” for that RBM size).
...
* CD(*n) can be seen as approximately following the gradient of (KL(p_0|p*\infty)-KL(p_n|p_\infty)). 
...
If you want, paste your current “notes template” (the headings/fields you like to keep across papers), and I’ll reshape this distillation to match it exactly—without losing any of the above content.
