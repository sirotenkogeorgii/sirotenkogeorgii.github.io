...
$\textbf{Definition (Wiener process):}$ A **Wiener process with a parameter $\sigma > 0$** is a stochastic process $W = \lbrace W_t, t \ge 0\rbrace$ defined by these properties:
...
*proof:*
$$\text{Cov}(Y)_{ij} = \text{Cov}(Y_i,Y_j) = \text{Cov}(W(t_i),W(t_j)) = (A\text{Cov}(Z)A^\top)_{ij}$$
$$= \sigma^2\sum_{k=1}^{\min(i,j)}\Delta t_k = \sigma^2\min(t_i,t_j)$$
...
$\textbf{Definition (Gaussian Process):}$ A stochastic process $\lbrace X(t),t\ge0 \rbrace$ is Gaussian, if for any $n\ge 1$ and time point $0 \le t_1 < \dots < t_n$ vector $(X(t_1),\dots,X(t_n))$ is a normal random vector.

$\textbf{Theorem (Wiener Process):}$ A stochastic process $\lbrace W(t),t\ge 0 \rbrace$ is a Wiener process *if and only if*
...
(X^{(n)}*{t_1},\dots,X^{(n)}*{t_k}) \Rightarrow (X_{t_1},\dots,X_{t_k})
...
> **Remark:** The "iterated logarithm" is the $\log\log n$ term: it grows extremely slowly, but it’s the precise correction needed to describe the almost-sure extremal oscillations.
...
$\textbf{Corollary (LIL for Wiener Process):}$ Let $W_t$ be a Wiener process and fix $\Delta>0$. Define increments
...
> **Remark:** The law of the iterated logarithm (LIL) is an **asymptotic result**, so it’s fundamentally about what happens as $n\to\infty$ (or $t\to\infty$ for Brownian motion).
...
$\textbf{Theorem (Bachelier):}$ For any $T$, $x\ge 0$:
