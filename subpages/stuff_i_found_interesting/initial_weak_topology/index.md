...
A neighborhood base at $0$ consists of the convex sets $\lbrace x:\|\varphi_1(x)\|<\varepsilon,\dots,\|\varphi_n(x)\|<\varepsilon\rbrace$ for finite $\lbrace\varphi_i\rbrace\subset E^\ast$ and $\varepsilon>0$. The convergence form is the one you already use as the definition of **weak convergence**:
...
With that axis in hand, the whole weak-topology story is a single sentence: **the norm topology sits too far toward the fine end.** It has Hausdorff separation to spare — and pays for that surplus with a ball that is not compact. The remedy is to slide back toward the coarse end until the ball becomes compact, *without sliding so far that you stop separating points*. That landing spot is the weak topology: coarse enough to compactify the ball (in reflexive spaces; unconditionally for duals via Banach–Alaoglu), yet still Hausdorff because $E^*$ separates points — and that Hausdorffness is exactly the residue of "don't coarsen *past* separation," secured by Hahn–Banach.
