...
$\textbf{Definition (Brownian Motion):}$ A stochastic process $\lbrace B_t\rbrace_{t \ge 0}$ whose finite-dimensional distributions are given by the construction above is called a Brownian motion starting at $x$. By construction, $P^x(B_0=x)=1$.
...
$\textbf{Definition (Version of a Stochastic Process):}$ A process $\lbrace X_t\rbrace$ is a version (or modification) of a process $\lbrace Y_t\rbrace$ if for all $t$:  
...
$\textbf{Theorem/Lemma (Kolmogorov's Continuity Theorem):}$ Suppose a process $\lbrace X_t\rbrace_{t \ge 0}$ satisfies the condition that for all $T > 0$, there exist positive constants $\alpha, \beta, D$ such that for 

$$0 \le s, t \le T:  E[\|X_t - X_s\|^\alpha] \le D \cdot \|t - s\|^{1+\beta}$$
