...
The sparsity induced by L1 regularization is a critical concept in embedded ML. A sparse model has many zero-valued weights. This means the corresponding multiply-accumulate operations during inference can be skipped entirely. This leads to:
...
**Glossary: Compressed Sparse Row (CSR)**  
A format for storing sparse matrices that avoids storing zero elements. It uses three arrays:
...
| P_eff | Effective sparse throughput, which is often much lower than P. |
| B | Total bytes moved for the dense operation. |
| BW | Memory bandwidth (bytes/sec). |
| B_sparse | Bytes moved for the sparse operation, including metadata (B_{sparse} = B_{activations} + B_{non-zero\_weights} + B_{metadata}). |
| T_overhead | Time cost for packing, format conversion, and handling load imbalance. |
