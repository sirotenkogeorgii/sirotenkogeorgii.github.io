...
Potential For a configuration with m tokens at positions $d_1$, $\dots$, $d_m$, the potential $\Phi$ is defined as: $\Phi$ = $\sum_{i=1}^m$ $\frac{1}{2^{d_i}}$
...
Strategy A strategy for a player is a function that determines their next move for every possible configuration where it is their turn. A winning strategy is one that guarantees a win regardless of the opponent's moves.
...
Determined Game A game is determined if one of the two players possesses a winning strategy.
...
Expected Weight of a Cut Let G be a graph with m edges. On input G, the expected weight of the cut returned by Algorithm Cut is $\frac{m}{2}$. Consequently, every graph G possesses a cut with a weight of at least $\frac{m}{2}$.
...
Expected Weight in Hypergraphs For a k-regular hypergraph with m edges, the expected weight of the cut is (1 - $\frac{2}{2^k}$)m.
...
Efficiency of Randomized Byzantine Agreement In a group of n processors where at most $\frac{n}{8}$ processors are malicious and the remainder follow the ByzantineAgreement protocol:
...
Possible Partners A woman F is considered possible for a man M if there exists at least one stable marriage where M and F are paired.
...
Optimality of the Proposal Algorithm The marriage H produced by the Proposal Algorithm is the least element with respect to the $\le_M$ relation. This means every man is married to the best possible woman available to him in any stable marriage.
...
Average-Case Complexity The average number of proposals made by the Proposal Algorithm on inputs of order n is O(n $\log$ n).
...
Geometric Distribution A random variable X follows a geometric distribution with probability p if $\mathbb{P}[X = i]$ = (1-p)^{i-1}p for i > 0. The expected value is $\mathbb{E}[X]$ = $\frac{1}{p}$.
...
Harmonic Numbers The n-th harmonic number is $H_n$ = $\sum_{i=1}^n$ $\frac{1}{i}$. It is bounded by $\ln$ n $\le$ $H_n$ $\le$ $\ln$ n + 1.
...
Expected Waiting Time of the Coupon Collector In the coupon collector's problem with n types, the expected number of rounds to collect all types is $nH_n$, which is O(n $\log$ n).
...
PAC Learner Let E be any set and let $\mathcal{K}$ be a subset of the power set $2^E$ of E. The learner A learns the concept class $\mathcal{K}$ probably approximately correct (PAC-learns) $\mathcal{K}$ with sample complexity s if:
...
Consistent Learner A concept K is consistent with data ($x_1$, $b_1$), $\dots$, ($x_s$, $b_s$) if K($x_j$) = $b_j$ for all j=1, $\dots$, s. A learner for a concept class $\mathcal{K}$ is consistent if it always yields a candidate concept consistent with the given data.
...
The VC dimension of $\mathcal{K}$, denoted VC($\mathcal{K}$), is the maximum size of a finite set X that is shattered by $\mathcal{K}$.
...
VC Dimension of Rectangles To see why VC($\mathcal{K}$) = 4 for axis-parallel rectangles:
...
A sorting algorithm where the input list can only be accessed by queries of the form "is $x_i$ < $x_j$?".
...
Upper Bound for Empty Columns There exists a black-box Las Vegas algorithm that decides if a Boolean matrix has an empty column with an expected worst-case cost of: $g_{upper}(n)$ = $\frac{n(n+1)}{2}$
...
Lower Bound for Empty Columns For any black-box Las Vegas algorithm for the empty column problem, the expected worst-case cost is at least: $g_{lower}(n)$ = $\frac{n(n+1)}{2}$
...
$\max_\tau$ $\mathbb{E}[k(A_{\sigma^*}, I_\tau)]$ = $\min_\sigma$ $\max_\tau$ $\mathbb{E}[k(A_\sigma, I_\tau)]$ A mixed strategy $\tau^*$ for Player II is optimal if: $\min_\sigma$ $\mathbb{E}[k(A_\sigma, I_{\tau^*})]$ = $\max_\tau$ $\min_\sigma$ $\mathbb{E}[k(A_\sigma, I_\tau)]$

</div>

In every matrix game, optimal strategies exist because the sets of mixed strategies are compact and payoffs are bounded. Let $k_1^*$ be the value Player I can enforce (the minimum possible maximum) and $k_2^*$ be the value Player II can enforce (the maximum possible minimum). It is always true that $k_2^*$ $\le$ $k_1^*$.
...
RandQuicksort Given a list S = ($s_{\pi(1)}$, $\dots$, $s_{\pi(n)}$) of n pairwise distinct items:
...
Upper Bound for RandQuicksort When RandQuicksort is run on any list of n pairwise distinct items, the expected number of comparisons is at most 1.4n $\log$ n.
...
Lower Bound for Average Case Let A be a deterministic black-box sorting algorithm. For any $\epsilon$ > 0 and almost all n, the average number of comparisons on inputs of size n is at least (1 - $\epsilon$)n $\log$ n.
...
Success Probability of Contract If algorithm Contract is run on a graph with n nodes, any specific minimum cut of the graph is output with probability at least $\frac{2}{n^2}$.
...
Probability of Reaching the Origin Let $p_j$ be the probability that a walk starting at $X_0$ = j eventually reaches 0.
...
The binary entropy function H : (0, 1) $\to$ [0, 1] is defined as: H($\rho$) = -$\rho$ $\log$ $\rho$ - (1 - $\rho$) $\log$ (1 - $\rho$)
...
Let $\rho$ = r/n and v(n, r) = $2^{H(\rho)n}$. For sufficiently large n and r $\le$ n/2: $\frac{v(n, r)}{n}$ $\le$ vol(n, r) $\le$ v(n, r)
...
The efficiency gain of local search over naive exhaustive search within the ball B($\sigma$, n, r) occurs when $a_k(n, r)$ < vol(n, r). To find the optimal radius, we minimize the ratio of the search tree size to the ball volume. Let $\rho$ = r/n. We approximate the ratio as: $\frac{a_k(n, r)}{vol(n, r)}$ $\le$ $\frac{k^{rn}}{2^{H(\rho)n} / n}$ = n $\cdot$ $2^{(\rho \log k - H(\rho))n}$
...
If we choose an initial assignment $\sigma$ uniformly at random, the probability that a specific satisfying assignment $\sigma_0$ falls within B($\sigma$, n, r) is $\frac{vol(n, r)}{2^n}$.

A randomized algorithm can be defined by repeating the local search t $\cdot$ $\frac{2^n}{vol(n, r)}$ times with independent random starting assignments. The probability of failing to find a satisfying assignment is at most (1 - $\frac{vol(n, r)}{2^n}$)^{t $\cdot$ $2^n$ / vol(n, r)} $\le$ $e^{-t}$.

For r = $\frac{n}{k+1}$ and t = $n^2$, the total number of assignments checked is approximately: poly(n) $\cdot$ $\left(\frac{2k}{k+1}\right)^n$ For 3-SAT (k=3), this results in a randomized algorithm with a running time of poly(n) $\cdot$ $1.5^n$ and an error probability of at most $2^{-n^2}$.
...
* Initialization: A and B publish a prime p and a generator d of $\mathbb{Z}_p^*$.
...
Edge Bound for Planar Graphs For a planar graph G with n nodes and m > 1 edges: m $\le$ 3n - 6
