...
* Probability Measure ((P)): A function (P: \mathcal{A} \to [0, 1]) that assigns a likelihood to each event in the (\sigma)-algebra. It must satisfy:
  1. (P(\emptyset) = 0).
  2. (P(\Omega) = 1).
  3. For any sequence of pairwise disjoint events (A_1, A_2, \dots), the probability of their union is the sum of their probabilities: (P(\bigcup_{i=1}^{\infty} A_i) = \sum_{i=1}^{\infty} P(A_i)).

The pair ((\Omega, \mathcal{A})) is called a measurable space.

Exercises

1. Consider an experiment of tossing a coin twice. Define the sample space (\Omega), the power set (\mathcal{P}(\Omega)) as the (\sigma)-algebra, and a valid probability measure for a fair coin.
...
The mapping itself is deterministic; the randomness is induced by the underlying probability measure on (\Omega). RVs induce probability distributions on the real line.

* Example (Coin Toss): For a coin toss where (\Omega = {Heads, Tails}), we can define an RV (X) such that (X(Heads) = 1) and (X(Tails) = 0).
...
Definition (Probability Mass Function - PMF): Let (a_1, a_2, \dots) be the values a discrete RV (X) can take. A function (f: \mathbb{R} \to [0, 1]) is a PMF if (f(x) = P(X=x)). Specifically, if (p_k = P(X=a_k)) with (\sum_k p_k = 1), then: [ f(x) = \begin{cases} p_k & \text{if } x = a_k \ 0 & \text{otherwise} \end{cases} ] The CDF for a discrete RV is a step function: (F(t) = \sum_{k: a_k \le t} p_k).
...
Poisson	(\lambda > 0)	(\frac{\lambda^x e^{-\lambda}}{x!}) for (x \in {0, 1, 2, \dots})	Models the number of events occurring in a fixed interval of time or space.
...
Normal	(\mu \in \mathbb{R}, \sigma^2 > 0)	(\frac{1}{\sqrt{2\pi\sigma^2}} \exp\left(-\frac{(x-\mu)^2}{2\sigma^2}\right))
Multivariate Normal	(\mu \in \mathbb{R}^d, \Sigma \in \mathbb{R}^{d \times d})	(\frac{1}{(2\pi)^{d/2}
Exponential	(\lambda > 0)	(\lambda e^{-\lambda x}) for (x \ge 0)
Uniform	(a, b \in \mathbb{R}, a < b)	(\frac{1}{b-a}) for (x \in [a, b])
...
* The variance measures the spread of the distribution: (Var(X) = E[(X - E[X])^2] = E[X^2] - (E[X])^2).
...
* Conditional Probability: The probability of (x) occurring given that (y) has occurred. [ P(x|y) = \frac{P(x, y)}{P(y)} ]
* Chain Rule: Decomposes a joint probability into a product of conditional probabilities. [ P(x_1, x_2, \dots, x_n) = P(x_1) P(x_2|x_1) P(x_3|x_1, x_2) \dots P(x_n|x_1, \dots, x_{n-1}) ]
* Marginalization: The probability of one variable can be found by summing (or integrating) over all possible values of other variables. [ P(x) = \sum_y P(x, y) = \sum_y P(x|y)P(y) ]
* Bayes' Rule: Relates a conditional probability to its inverse. It is the foundation of Bayesian inference. [ P(y|x) = \frac{P(x|y)P(y)}{P(x)} ]
* Independence: Two RVs (X) and (Y) are independent if learning the value of one provides no information about the other. This holds if and only if: [ P(x, y) = P(x)P(y) \quad \text{or equivalently} \quad P(x|y) = P(x) ]

3.2 Asymptotic Theorems

These theorems describe the behavior of the sum of a large number of random variables.

Theorem (Strong Law of Large Numbers - SLLN): Let ((X_i){i=1}^\infty) be a sequence of independent and identically distributed (i.i.d.) random variables with finite mean (E[X_i] = \mu). Then the sample mean converges almost surely to the true mean:[ \frac{1}{n} \sum{i=1}^n X_i \xrightarrow{a.s.} \mu \quad \text{as } n \to \infty ]

Theorem (Central Limit Theorem - CLT): Let ((X_i){i=1}^\infty) be a sequence of i.i.d. random variables with mean (\mu) and finite variance (\sigma^2). Then the distribution of the standardized sample mean converges to a standard Normal distribution:[ \frac{\sqrt{n}(\frac{1}{n}\sum{i=1}^n X_i - \mu)}{\sigma} \xrightarrow{d} \mathcal{N}(0, 1) \quad \text{as } n \to \infty ]

Key Takeaway: The Power of the CLT

The Central Limit Theorem is remarkable because it holds regardless of the underlying distribution of the (X_i). It explains why the Normal distribution appears so frequently in nature and statistics. In machine learning, it provides justification for assuming that errors or noise in many processes are normally distributed, simplifying modeling and analysis.
...
* Bias: The difference between the expected value of the estimator and the true parameter: (Bias(\hat{\theta}) = E[\hat{\theta}] - \theta). An estimator is unbiased if its bias is zero.
...
Let ((x_i, y_i)) be (N) data pairs. We approximate the true relationship (y=f(x)) with a parameterized function (f_\theta(x)). The residual for the (i)-th data point is (\epsilon_i = y_i - f_\theta(x_i)). The LS estimator is: [ \hat{\theta}{LS} = \arg\min{\theta} \sum_{i=1}^N \epsilon_i^2 = \arg\min_{\theta} \sum_{i=1}^N (y_i - f_\theta(x_i))^2 ]
...
* Example: Estimating the Population Mean
  ...
  * Derivation: To find the minimum, we differentiate with respect to (\mu) and set to zero. [ \frac{\partial S}{\partial \mu} = \sum_{i=1}^N -2(X_i - \mu) = -2 \left( \sum X_i - N\mu \right) = 0 ] [ \sum X_i - N\mu = 0 \implies \hat{\mu} = \frac{1}{N} \sum_{i=1}^N X_i ]
...
Let (X = {x_1, \dots, x_N}) be observed data drawn from a model with density (p(x|\theta)).

* Likelihood Function: The joint density of the observed data, viewed as a function of the parameter (\theta): [ \mathcal{L}(\theta | X) = p(x_1, \dots, x_N | \theta) ]
* If the data are i.i.d., the likelihood factorizes: [ \mathcal{L}(\theta | X) = \prod_{i=1}^N p(x_i | \theta) ]
* MLE Definition: The MLE is the value of (\theta) that maximizes the likelihood function: [ \hat{\theta}{MLE} = \arg\max{\theta \in \Theta} \mathcal{L}(\theta | X) ] In practice, it is often easier to maximize the log-likelihood function, (\ell(\theta|X) = \log \mathcal{L}(\theta|X)), since the logarithm is a monotonic function and has the same maximum.
* Example: Estimating the Mean of a Normal Distribution
  * Model: (X_i \sim \mathcal{N}(\mu, \sigma^2)) i.i.d., with (\sigma^2) known.
  * Log-Likelihood: [ \ell(\mu | X) = \log \prod_{i=1}^N \frac{1}{\sqrt{2\pi\sigma^2}} \exp\left(-\frac{(X_i-\mu)^2}{2\sigma^2}\right) = \sum_{i=1}^N \left( -\frac{1}{2}\log(2\pi\sigma^2) - \frac{(X_i-\mu)^2}{2\sigma^2} \right) ]
  * Maximization: Differentiate with respect to (\mu) and set to zero. [ \frac{\partial \ell}{\partial \mu} = \sum_{i=1}^N -\frac{-2(X_i-\mu)}{2\sigma^2} = \frac{1}{\sigma^2} \sum_{i=1}^N (X_i-\mu) = 0 ] [ \sum X_i - N\mu = 0 \implies \hat{\mu}{MLE} = \frac{1}{N} \sum{i=1}^N X_i ]
  * For this model, the MLE and LS estimates for the mean are identical.

5.3 Bayesian Inference (BI)

Bayesian inference uses Bayes' rule to update knowledge about parameters after observing data. It recovers a full posterior distribution for the parameters, not just a point estimate.

The core of Bayesian inference is Bayes' rule: [ p(\theta | X) = \frac{p(X | \theta) p(\theta)}{p(X)} ] Where:

* (p(\theta | X)) is the posterior distribution: our belief about (\theta) after seeing data (X).
* (p(X | \theta)) is the likelihood: same as in MLE.
* (p(\theta)) is the prior distribution: our belief about (\theta) before seeing any data.
* (p(X) = \int p(X|\theta)p(\theta)d\theta) is the evidence or marginal likelihood of the data.

This can be written as: Posterior (\propto) Likelihood (\times) Prior.

* Example: Inferring the mean (\mu) of a Normal distribution ((\sigma^2) known) with a Normal prior on (\mu).
  * Likelihood: (p(X|\mu) \propto \exp\left(-\frac{1}{2\sigma^2}\sum(x_i - \mu)^2\right))
  * Prior: (\mu \sim \mathcal{N}(\mu_0, \sigma_0^2)), so (p(\mu) \propto \exp\left(-\frac{(\mu - \mu_0)^2}{2\sigma_0^2}\right))
  * The posterior (p(\mu|X)) will also be a Normal distribution.
  * A challenge in Bayesian inference is that the evidence (p(X)) is often an intractable integral, requiring numerical methods like MCMC or variational inference.
...
2. Repeat for iterations (n=0, 1, 2, \dots): [ \theta_{n+1} = \theta_n - \gamma \nabla J(\theta_n) ] where (J(\theta)) is the cost function and (\gamma > 0) is the learning rate.
3. Stop when convergence is reached (e.g., (|\nabla J(\theta_n)| < \epsilon)).

* Example: GD for LS Estimation of the Mean
  * Cost: (J(\mu) = \sum (X_i - \mu)^2)
  * Gradient: (\nabla J(\mu) = \frac{\partial J}{\partial \mu} = -2\sum (X_i - \mu))
  * Update Rule: (\mu_{n+1} = \mu_n - \gamma (-2\sum(X_i - \mu_n)) = \mu_n + 2\gamma\sum(X_i - \mu_n))
...
The Newton-Raphson method is a second-order optimization algorithm that uses the curvature of the loss landscape to take more intelligent steps. It adapts the learning rate by incorporating the Hessian matrix (the matrix of second partial derivatives).

Update Rule: [ \theta_{n+1} = \theta_n - H_{f(\theta_n)}^{-1} \nabla f(\theta_n) ] where (H_{f(\theta_n)}) is the Hessian matrix of the function (f) evaluated at (\theta_n). The term (H^{-1}) acts as an adaptive, matrix-valued learning rate. While it can converge much faster than GD, computing and inverting the Hessian is computationally expensive for high-dimensional parameter spaces common in deep learning.
